...
public partial class 按键脚本 : Node2D
{
	...
	public override void _Input(InputEvent @event)
	{
		...
		if (@event is InputEventMouseButton mouseButton)
		{
			...
			{
				worker.WalkTo(坐标转换器.ToMatrixPosition(new Vector2(GetGlobalMousePosition().X, GetGlobalMousePosition().Y), camera.cameraHight));
			}
			if (Input.IsMouseButtonPressed(MouseButton.Right))
			{
				GD.Print(坐标转换器.ToMatrixPosition(new Vector2(GetGlobalMousePosition().X, GetGlobalMousePosition().Y), camera.cameraHight));
			}
		}
	}
